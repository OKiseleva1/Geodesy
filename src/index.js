...
async function loadEquipments() {
    return (await fetch("./src/equipments.json")).json();
}
                ...
                popup.innerHTML += `
                    ...
                    <div class="popup-description">
                        ...
                        <ul class="popup-list">
                            ...
                            <li class="popup-item"><span>Габаритные размеры: </span>${equipments[path].size}</li>
                            <li class="popup-item"><span>Точность измерения: </span>${equipments[path].accuracy}</li>
                            </ul>
                    </div>`;
...
CAROUSEL.addEventListener("animationend", (animationEvent) => { 
    ...
    if(animationEvent.animationName === "move-left"){
        ...
        for (let i = 0; i < 3; i++) {
            ...
            card.setAttribute("id", `${equipments[key[i]].id}`);
            card.innerHTML += `
                        <img src="${equipments[key[i]].img}" alt="${equipments[key[i]].name}">
                        <h3 class="products__text">${equipments[key[i]].name}</h3>
